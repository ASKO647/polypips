...
import {
  SIGNAL_ANALYSIS_LOADING_STEPS,
  SIGNAL_ANALYSIS_STEP_ORDER,
  type SignalAnalysisProgressStep,
} from "@/lib/data/signal-analysis";
import { cn } from "@/lib/utils";

/** Smart Wallets universe's own loading state — same visual rhythm as
 * analyse-ia/analysis-loading.tsx and sports' own, a distinct component so
 * neither of those two stay untouched. */
export function SignalAnalysisLoading({
  currentStep,
}: {
  currentStep: SignalAnalysisProgressStep | null;
}) {
  const currentIndex = currentStep ? SIGNAL_ANALYSIS_STEP_ORDER.indexOf(currentStep) : -1;
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-8 rounded-2xl border border-white/10 bg-white/[0.03] px-6 py-16">
      ...
      <div className="flex w-full max-w-sm flex-col gap-3">
        {SIGNAL_ANALYSIS_STEP_ORDER.map((step, i) => {
          ...
          return (
            <div key={step} className="flex items-center gap-3">
              ...
              >
                {SIGNAL_ANALYSIS_LOADING_STEPS[step]}
              </span>
            </div>
          );
        })}
