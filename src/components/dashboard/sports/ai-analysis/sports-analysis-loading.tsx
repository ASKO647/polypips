...
import {
  SPORTS_ANALYSIS_LOADING_STEPS,
  SPORTS_ANALYSIS_STEP_ORDER,
  type SportsAnalysisProgressStep,
} from "@/lib/data/sports-analysis";
import { cn } from "@/lib/utils";

/** Sport universe's own loading state — same visual rhythm as
 * analyse-ia/analysis-loading.tsx (Polymarket's), but a distinct
 * component so that feature stays untouched. */
export function SportsAnalysisLoading({
  currentStep,
}: {
  currentStep: SportsAnalysisProgressStep | null;
}) {
  const currentIndex = currentStep ? SPORTS_ANALYSIS_STEP_ORDER.indexOf(currentStep) : -1;
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center gap-8 rounded-2xl border border-white/10 bg-white/[0.03] px-6 py-16">
      ...
      <div className="flex w-full max-w-sm flex-col gap-3">
        {SPORTS_ANALYSIS_STEP_ORDER.map((step, i) => {
          ...
          return (
            <div key={step} className="flex items-center gap-3">
              ...
              >
                {SPORTS_ANALYSIS_LOADING_STEPS[step]}
              </span>
            </div>
          );
        })}
