...
const REASONS = ["Spam", "Contenu inapproprié", "Harcèlement", "Autre"];
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
      <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-[#0f0808] p-6 shadow-[0_20px_60px_-16px_rgba(0,0,0,0.6)]">
        <div className="flex items-center justify-between gap-4">
          <h2 className="font-display text-base font-bold text-white">Signaler ce message</h2>
          <button
            type="button"
            onClick={onClose}
            aria-label="Fermer"
        ...
        {sent ? (
          <p className="mt-5 text-sm text-emerald-400">
            Signalement envoyé. Merci de nous aider à garder la communauté saine.
          </p>
        ) : (
