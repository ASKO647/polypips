...
/** Real timestamps for this user's sport analyses in the last `days` days —
 * same role as analyses.ts's fetchAnalysisTimestamps, for the dashboard
 * overview's "Analyse IA Sport" sparkline and windowed activity counts. */
export async function fetchSportAnalysisTimestamps(
  supabase: SupabaseClient,
  userId: string,
  days: number
): Promise<string[]> {
...
export async function countSportAnalysesAllTime(
  supabase: SupabaseClient,
  userId: string
): Promise<number> {
  const { count, error } = await supabase
